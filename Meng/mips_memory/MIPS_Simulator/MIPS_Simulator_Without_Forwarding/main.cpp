...
struct instruction_circular_buffer{
	...
	signed long forward_rs_value;
	signed long forward_rt_value;
	...
	instruction_circular_buffer() : 
		stage(0),
		hex_instr(0),
		forward_rs_value(NULL),
		forward_rt_value(NULL),
		no_op(false){}
...
void reset_buffer(instruction_circular_buffer & buffer)
{
	...
	buffer.no_op = false;
	buffer.forward_rs_value = NULL;
	buffer.forward_rt_value = NULL;
}
	...
	while (!HALT)
	{
		...
		for(int x =0; x < current_circular_buff_count; x++){
			if(!hazard){
				...
				/*if(stall == 1 && instr_circular_buffer[x].stage == 1){
		...
		while(circular_buffer_pos < current_circular_buff_count && !abort)
		{
			...
			switch (stage)
			{
				...
				case 1:
					...
					if(!instr_circular_buffer[circular_buffer_pos].no_op && (stall <= 0 || stall >= 2)){
				...
				case 2: 
					...
					if(!hazard && !instr_circular_buffer[circular_buffer_pos].no_op)
					{
						...
						do{
							if(instr_circular_buffer[tempCounter].stage == 3 
								|| instr_circular_buffer[tempCounter].stage == 4){
								found = true;
							}
						...
						if(found){
							// true - J type
							// false - I type
							if(instr_circular_buffer[tempCounter].parts.insr_type){
								temprd = instr_circular_buffer[tempCounter].parts.rd;
							}else{ // I type
								temprd =instr_circular_buffer[tempCounter].parts.rt;
							}
							...
							if(instr_circular_buffer[circular_buffer_pos].stage > 0){
								if( instr_circular_buffer[circular_buffer_pos].parts.insr_type){ // J type
									tempsource1 = instr_circular_buffer[circular_buffer_pos].parts.rs;
									tempsource2 = instr_circular_buffer[circular_buffer_pos].parts.rt;
						...
						// if there is a hazard stall 3 cycles for the ID and IF stages 
						// stall = 3 because of the pre-drecrement
						if(hazard) {
							my_dump.total_data_hazard += 1;

							if(instr_circular_buffer[tempCounter].parts.opcode == 3){
								stall = 3;
							}else{
								stall = 2;
							}
